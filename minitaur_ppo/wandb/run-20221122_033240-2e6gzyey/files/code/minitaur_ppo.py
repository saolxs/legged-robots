...
def make_env(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = gym.wrappers.TransformObservation(env, lambda obs: np.clip(obs, -5, 10))
        env = gym.wrappers.ContinuousToDiscrete(env)
        #calculate the discounted return and return the normalized return
        env = gym.wrappers.NormalizeReward(env)
        #clip the normalized reward between -10 and 10
        env = gym.wrappers.TransformReward(env, lambda reward: np.clip(reward, -5, 10))
        env.seed(seed)
