...
class Agent(nn.Module):
    ...
    def get_action_value(self, x, action=None):
        #unnormalised action probabilities
        #action_probs = logit
        ...
        return action, action_dist.log_prob(action), action_dist.entropy(), action_probs
...
def parse_args():
        ...
        parser.add_argument("--anneal-lr", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True,
                            help="Toggle learning_rate annealing for policy and value networks")
...
def main():
    ...
    envs = gym.vector.SyncVectorEnv(
        [gym_env(args.gym_id, args.seed + i, i, args.capture_video, run_name) for i in range(args.num_envs)])
    
    assert isinstance(envs.single_action_space, gym.spaces.Discrete), "only discrete action space is supported"
    print("env.single_observation_space.shape:", envs.single_observation_space.shape)
    print("env.single_action_space.n:", envs.single_action_space.n)
    ...
    nxt_obs = torch.Tensor(envs.reset()).to(device)
    nxt_done = torch.zeros(args.num_envs).to(device)
    num_updates = args.total_timesteps // args.batch_size
    
    print(num_updates)
    print("nxt_obs.shape", nxt_obs.shape)
    print("agent.forward(nxt_obs)", agent.forward(nxt_obs))
    print("agent.forward(nxt_obs).shape", agent.forward(nxt_obs).shape)
    print()
    print("agent.get_action_value(nxt_obs)", agent.get_action_value(nxt_obs))
    ...
    for update in range(1, num_updates+1):
        ...
        for step in range(0, args.num_steps):
            ...
            #action logic
            with torch.no_grad():
                action,log_prob, _, value = agent.get_action_value(nxt_obs)
                value[step] = value.flatten()
            action[step] = action
            log_prob[step] = log_prob
            
            #execute game and log data
            nxt_obs, reward, done, info = envs.step(action.cpu().numpy())
            rewards[step] = torch.tensor(reward).to(device).view(-1)
            nxt_obs, nxt_done = torch.Tensor(nxt_obs).to(device), torch.Tensor(done).to(device)
            ...
            with torch.no_grad():
                nxt_value = agent.get_action_value(nxt_obs).reshape(1, -1)
                if args.gae:
                    advantages = torch.zeros_like(rewards).to(device)
                    lst_gae_lam = 0
                    for t in reversed(range(args.num_steps)):
                        if t==args.num_steps - 1:
                            nxt_non_terminal = 1.0 - nxt_done
                            nxt_values = nxt_value
                        else:
                            nxt_non_terminal = 1.0 - dones[t+1]
                            nxt_values = values[t+1]
                        delta = rewards[t] + args.gamma * nxt_non_terminal - values[t]
                        advantages[t] = lst_gae_lam = delta + args.gamma * args.gae_lambda * nxt_non_terminal * lst_gae_lam
                    ...
                    for t in reversed(range(args.num_steps)):
                        if t==args.num_steps - 1:
                            ...
                            nxt_non_terminal = 1.0 - dones[t+1]
                            nxt_return = returns[t+1]
                        delta = rewards[t] + args.gamma * nxt_non_terminal - values[t]
                        returns[t] = rewards[t] + args.gamma * nxt_non_terminal * nxt_return
                    advantages = returns - values
        
            # flatten the batch
            batch_obs = obs.reshape((-1,) + envs.single_observation_space.shape)
            batch_logprobs = logprobs.reshape(-1)
            batch_actions = action.reshape((-1,) + envs.single_action_space.shape)
